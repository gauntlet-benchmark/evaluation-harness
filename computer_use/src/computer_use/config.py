...
MODEL_REGISTRY: dict[str, ModelSpec] = {
    # Anthropic first-party API rates. An unregistered model gets no ModelSpec,
    # which silently disables token tracking and cost accounting entirely
    # (agent.py only builds a TokenUsageTracker when a spec is found) — so a
    # model must be registered here before it can be cost-tracked.
