...
class OpenAIProvider(LLMProvider):
    ...
    def _field(action: Any, name: str, default: Any = None) -> Any:
        """Read one field off an action, whether it is a dict or an SDK object.

        The Responses API returns computer actions inside ``computer_call.actions``
        as PLAIN DICTS, while the older singular ``computer_call.action`` came
        back as a typed SDK object. Reading them with ``getattr`` alone silently
        yields the default for every dict field -- which made every action look
        like an unknown type and fall through to the screenshot fallback below,
        so the agent screenshotted forever and never clicked anything. Runs
        still "completed" and answered, so nothing failed loudly; see
        scripts/archive/gpt54_cu_noop_screenshots_*.
        """
    ...
    def _translate_action(self, action: Any, index: int) -> list[ContentBlock]:
        ...
        action_type = self._field(action, "type", "unknown")
        ...
        if action_type == "click":
            ...
            return [
                self._action_block(
                    ...
                    coordinate=[self._field(action, "x", 0), self._field(action, "y", 0)],
                )
            ]
        ...
        if action_type == "drag":
            path = self._field(action, "path", [])
            if len(path) >= 2:
                start = path[0]
                end = path[-1]
                start_coord = [self._field(start, "x", 0), self._field(start, "y", 0)]
                end_coord = [self._field(end, "x", 0), self._field(end, "y", 0)]
            else:
                start_coord = [0, 0]
                end_coord = [0, 0]
            return [
        ...
        # Falling back to a screenshot is a no-op the agent cannot distinguish
        # from a deliberate look, so an unhandled action type used to burn a
        # whole run silently. Say so.
        print(
            f"[openai_provider] WARNING: unhandled computer action {action_type!r} "
            f"-> falling back to screenshot; raw={action!r}"
        )
        return [self._action_block(call_id, index, action="screenshot")]
    ...
    def _build_scroll_block(action: Any, call_id: str, index: int) -> ContentBlock:
        x = OpenAIProvider._field(action, "x", 512)
        y = OpenAIProvider._field(action, "y", 384)
        scroll_x = OpenAIProvider._field(action, "scroll_x", 0)
        scroll_y = OpenAIProvider._field(action, "scroll_y", 0)
    ...
    def _build_keypress_blocks(
        ...
        keys: list[str] = OpenAIProvider._field(action, "keys", []) or []
        if not keys:
            return [
                ContentBlock(
                    type="tool_use",
