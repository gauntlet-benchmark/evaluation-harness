"""CLI script to run the agent for multiple tasks from a YAML file."""
...
def load_tasks(path: str) -> list[dict[str, Any]]:
    ...
    Args:
        path: Path to YAML file with 'tasks' key.
...
def select_tasks(
    ...
    """Filter tasks by 1-based inclusive range.
    ...
    end_idx = end if end > 0 else len(tasks)
...
def run_single_task(
    ...
    Args:
        ...
        initial_delay: Seconds to wait after navigation before the agent's first
            action. Some apps serve a fully-loaded HTML shell while still
            fetching their data client-side -- the flightradar HF space sits on
            "Loading flight data..." for ~75s -- so an agent that starts
            immediately screenshots a spinner, concludes the data is
            unavailable, and answers "unknown" in 6 steps. The REAL harness
            spells this ``initial_delay`` per app; this is the same knob.
    ...
    try:
        ...
        if initial_delay > 0:
            console.print(f"  [dim]Initial delay: {initial_delay:g}s (waiting for the app to load)[/dim]")
...
def _extract_steps_from_messages(
    ...
    When a token tracker is supplied, each step also carries a ``stats`` dict
    using the same key names the main harness records in ``StepInfo.stats``
    (``input_tokens`` / ``output_tokens`` / ``total_tokens`` / ``step_elapsed``
    / ``agent_elapsed``), so per-step usage is readable by the same code that
    reads the rest of the results tree. One agent iteration is one API call is
    one assistant turn, so iterations line up with steps by index.

    Args:
        messages: Agent message history (user/assistant alternation).
        token_tracker: Optional TokenUsageTracker whose ``iterations`` hold the
            per-call usage. Absent (unregistered model), steps carry no stats.

    Returns:
        List of step dicts with action, model_response, err_msg fields.
    """
    iterations = list(getattr(token_tracker, "iterations", None) or [])
    ...
    for msg in messages:
        ...
        if step_idx < len(iterations):
            usage = iterations[step_idx]
            step_entry["stats"] = {
                "input_tokens": usage.input_tokens,
                "output_tokens": usage.output_tokens,
                "total_tokens": usage.total_tokens,
                "step_elapsed": usage.step_elapsed,
                "agent_elapsed": usage.agent_elapsed,
            }
...
def save_trajectory(
    ...
    steps = _extract_steps_from_messages(messages, token_tracker)
...
def run_all_tasks(
    ...
    *,
    ...
    Args:
        ...
        initial_delay: Seconds to wait before the agent's first action, per task.
    ...
    for i, tc in enumerate(tasks, 1):
        test_id: str = tc["id"]
        description: str = tc.get("description", "")

        console.rule(f"[bold cyan]Task {i}/{len(tasks)}: {test_id}[/bold cyan]")
        if description:
...
def print_summary_table(
    ...
    Args:
        console: Rich console for output.
        tasks: Original test case dicts.
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run computer-use agent on multiple tasks from a YAML file",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s --tasks tasks/circuit.yaml --start-url https://example.com
  %(prog)s --tasks tasks/circuit.yaml --start-url https://example.com --tasks-range 10:20
  %(prog)s --tasks tasks/circuit.yaml --start-url https://example.com \\
      --results-dir results/sonnet --post-run-url /finish
  %(prog)s --tasks tasks/video.yaml --start-url http://localhost:5173 \\
...
""",
    ...
    parser.add_argument(
        ...
        default=0.0,
        help=(
            "Seconds to wait after navigation before the agent's first action. "
            "Needed for apps that load data client-side behind a spinner "
            "(flightradar takes ~75s); without it the agent screenshots the "
            "spinner and answers 'unavailable'"
        ),
    )
...
def main() -> None:
    ...
    console.print(f"[dim]Tasks: {args.tasks}[/dim]")
    ...
    selected = select_tasks(all_tasks, args.tasks_range)

    if not selected:
        console.print("[bold red]No tasks matched the given range.[/bold red]")
        sys.exit(1)

    range_label = f"{args.tasks_range[0]}:{args.tasks_range[1]}" if args.tasks_range else "all"
    console.print(
        f"[dim]Tasks:      {len(selected)}/{len(all_tasks)} (range: {range_label})[/dim]"
    )

    yaml_stem = Path(args.tasks).stem
