...
class RichLogger:
    ...
    def task_complete(self, errored: bool = False, time_taken: float = None, task_id: str = None):
        """Print a task completion message.

        Runs are not scored here — success/failure comes from `evaluation/`
        afterwards — so this reports only whether the run itself errored.
        """
        self.print(f"[{Colors.SECONDARY}]─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ─── ──[/{Colors.SECONDARY}]")
        if errored:
            content = f"[{Colors.ERROR}]📊 Run errored[/{Colors.ERROR}]"
        else:
            content = f"[{Colors.SUCCESS}]📊 Run finished[/{Colors.SUCCESS}]"
