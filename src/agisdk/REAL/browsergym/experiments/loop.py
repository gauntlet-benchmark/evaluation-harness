    ...
    def run(self):
        ...
        try:
            ...
            if self.initial_delay > 0:
                waited = _wait_until_settled(env.unwrapped.page, budget=self.initial_delay)
                logger.info(
                    f"Waited {waited:.1f}s (budget {self.initial_delay}s) for the page to settle "
...
class StepInfo:
    ...
    reward: float
        The reward of the step. Always 0 here: the benchmark apps carry no
        in-environment success criteria, and are scored after the fact by
        ``evaluation/``.
    terminated: bool
...
def _save_summary_info(
    ...
    # The final page is only captured here when no post-run capture is
    # configured; otherwise the post_run_page_* fields below hold it.
...
def _extract_agent_response(step_info: Optional[StepInfo]) -> str:
    ...
    # The episode may have ended because the agent emitted its final answer
    # instead of an action; that answer is the response.
...
def _build_agent_outputs_payload(
    ...
    # ``post_run_page_html`` is deliberately not repeated here. It is the
    # largest field a run produces and nothing reads it from this file;
    # summary_info.json keeps the single copy.
    steps = []
    for step_info in episode_info:
        agent_info = step_info.agent_info or {}
        if not (
...
def _wait_until_settled(page, budget: float, poll: float = 2.0, stable_polls: int = 3) -> float:
    ...
    A fixed sleep cannot fit these apps: the Flight Analyser downloads its
    dataset on a cold context and has been measured anywhere between 1.6s and
    270s, so any constant is both too short and too long. Treat the configured
    delay as a *maximum* instead and watch the page — while an app is loading
    its body text keeps changing (spinners, "downloading — 37%"). Once that
    holds steady the app has settled.

    Text alone is not enough, though: a stalled progress bar reads as stable, so
    also require the network to be quiet. Apps that are ready immediately cost
    only ``poll * stable_polls`` seconds rather than the whole budget.
    ...
    events = (("request", _started), ("requestfinished", _ended), ("requestfailed", _ended))
    ...
    try:
        while time.time() - start < budget:
            try:
                ...
                current = None
            if inflight["n"] == 0 and current is not None and current == previous:
                unchanged += 1
...
def _capture_post_run_page(env, agent, post_run_url: str):
    # Reach the page through .unwrapped: gymnasium dropped attribute
    # forwarding on wrappers in 1.0, so `env.page` raises AttributeError on
    # modern versions and every capture would come back empty.
    page = env.unwrapped.page
    resolved_url = urljoin(page.url, post_run_url)
    capture_error = None
    ...
    # domcontentloaded fires before the client bundle hydrates, so reading the
    # page here is a race the fast deployments win and the slow ones lose —
    # losing it yields an empty export and an unscorable run even though the
    # agent did the work. These endpoints render their JSON into a <pre>, so
    # wait for that; fall back to network idle for any that do not. Never let
    # this step fail the capture.
    try:
    ...
    try:
        post_run_obs = env.unwrapped._get_obs()
        ...
        # The export endpoints render their payload as text inside a <pre>, and
        # the evaluators parse that payload as JSON. The observation only offers
        # pruned HTML / axtree, so taking content from there hands the evaluator
        # '<pre bid="2" style=...>{...}' instead of '{...}'. Prefer the page's
        # own text and keep the observation artifacts alongside it.
        try:
            page_text = page.locator("body").inner_text(timeout=2000)
            if page_text and page_text.strip():
                post_run_page_content = page_text
        except Exception:
            pass
