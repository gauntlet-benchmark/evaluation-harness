...
- **base.py**: Base class for the benchmark tasks. It navigates to the app, hands the
  agent its goal, and ends the episode once the agent replies. Runs are not scored here
  — see `evaluation/` at the repo root.

- **task_config.py**: Loads task definitions from the YAML suites in `tasks/`
