...
# Maps app dirname → tasks YAML filename stem.
APP_TO_YAML: dict[str, str] = {
    "graph":       "graph",
    "3d":          "3d",
    "video":       "video",
...
def load_difficulty_map(tasks_dir: Path) -> dict[str, str]:
    """Return {task_id: difficulty_level} for every task that declares one."""
    result: dict[str, str] = {}
    for yaml_name in APP_TO_YAML.values():
        p = tasks_dir / f"{yaml_name}.yaml"
        if not p.exists():
            continue
        try:
            ...
            continue
        for tc in (data or {}).get("tasks", []):
            tid = tc.get("id")
            diff = tc.get("difficulty_level")
            if tid and diff:
                result[str(tid)] = str(diff)
    return result
...
def _per_test_stats(model_dir: Path) -> dict[str, dict]:
    ...
    for run_dir in model_dir.iterdir():
        ...
        try:
            ...
            continue
        # Standard harness keys; fall back to Gemini Computer-Use agent keys.
        token_usage = d.get("token_usage") or {}
        tokens = d.get("stats.cum_total_tokens") or token_usage.get("total_token_count")
        elapsed = d.get("stats.cum_step_elapsed") or d.get("total_model_duration_seconds")
        out[test_id] = {
            "tokens":    tokens,
            "elapsed_s": elapsed,
...
def aggregate(
    ...
    """Return (success_table, eff_by_model, eff_by_model_app, eff_by_model_diff).

    * by_model_app[model][app] = {"runs": [pass_rate_per_run, ...]}
    * eff_by_model[model]      = {"tokens": [..per-test..], "elapsed": [...],
                                  "steps": [...]}                       (pooled overall)
    * eff_by_model_app[model][app] = same shape as eff_by_model, but per-app.
    * eff_by_model_diff[model][difficulty] = same shape, pooled across all apps
                                             that carry a difficulty_level field.
    """
    ...
    eff_by_model: dict[str, dict[str, list]] = defaultdict(
        lambda: {"tokens": [], "elapsed": [], "steps": []}
    )
    eff_by_model_app: dict[str, dict[str, dict[str, list]]] = defaultdict(
        lambda: defaultdict(lambda: {"tokens": [], "elapsed": [], "steps": []})
    )
    eff_by_model_diff: dict[str, dict[str, dict[str, list]]] = defaultdict(
        lambda: defaultdict(lambda: {"tokens": [], "elapsed": [], "steps": []})
    )
    dm = difficulty_map or {}
    ...
    for _run, app_dirname, model, path in triples:
        ...
        # Efficiency stats — pool overall, per-app, and per-difficulty.
        for test_id, st in stats.items():
            diff = dm.get(test_id)
            buckets = [eff_by_model[model], eff_by_model_app[model][app_dirname]]
            if diff:
                buckets.append(eff_by_model_diff[model][diff])
            for b in buckets:
    ...
    return by_model_app, eff_by_model, eff_by_model_app, eff_by_model_diff
...
def write_efficiency_diff_csv(
    out_path: Path,
    eff_by_model: dict,
    eff_by_model_diff: dict,
    by_model_app: dict,
) -> None:
    """Efficiency table broken down by difficulty level.

    Columns: Model | ACT (↓) | AWL (↓) | AS (↓) | SPF (↑)
             | ACT easy (↓) | AWL easy (↓) | AS easy (↓)
             | ACT medium (↓) | AWL medium (↓) | AS medium (↓)
             | ACT hard (↓) | AWL hard (↓) | AS hard (↓)

    The overall ACT/AWL/AS/SPF columns are identical to efficiency.csv.
    Tasks without a difficulty_level (flightradar, circuit) contribute only to
    the overall columns, not to any difficulty column.
    """
    # Overall per-model summaries (same as write_efficiency_csv).
    summaries: dict[str, dict[str, float]] = {}
    for model, vals in eff_by_model.items():
        tokens = vals["tokens"]
        elapsed = vals["elapsed"]
    ...
    diff_summaries: dict[str, dict[str, dict[str, float]]] = {}
    for model, by_diff in eff_by_model_diff.items():
        diff_summaries[model] = {}
        for diff, vals in by_diff.items():
            tokens = vals["tokens"]
            elapsed = vals["elapsed"]
            steps = vals["steps"]
            diff_summaries[model][diff] = {
                "act_mean":  statistics.mean(tokens) if tokens else math.nan,
                "act_std":   statistics.pstdev(tokens) if len(tokens) > 1 else (0.0 if tokens else math.nan),
                "awl_mean":  statistics.mean(elapsed) if elapsed else math.nan,
                "awl_std":   statistics.pstdev(elapsed) if len(elapsed) > 1 else (0.0 if elapsed else math.nan),
    ...
    headers = (
        ["Model", "ACT (↓)", "AWL (↓)", "AS (↓)", "SPF (↑)"]
        + [f"ACT {d} (↓)" for d in DIFFICULTY_LEVELS]
        + [f"AWL {d} (↓)" for d in DIFFICULTY_LEVELS]
        + [f"AS {d} (↓)"  for d in DIFFICULTY_LEVELS]
    )

    rows: list[list[str]] = []
    all_models = set(summaries) | set(diff_summaries)
    for model in sorted(all_models, key=lambda m: MODEL_DISPLAY.get(m, m)):
        s = summaries.get(model, {})
        spf = "—" if model not in pareto_input else ("1" if model in frontier else "0")
        row: list[str] = [
            MODEL_DISPLAY.get(model, model),
            _fmt_num(s.get("act_mean", math.nan), s.get("act_std", math.nan), digits=0),
            _fmt_num(s.get("awl_mean", math.nan), s.get("awl_std", math.nan), digits=1),
            _fmt_num(s.get("as_mean",  math.nan), s.get("as_std",  math.nan), digits=1),
            spf,
        ]
        ds = diff_summaries.get(model, {})
        for metric in ("act", "awl", "as"):
            digits = 0 if metric == "act" else 1
            for diff in DIFFICULTY_LEVELS:
                entry = ds.get(diff)
                if entry is None:
                    row.append("—")
                else:
                    row.append(_fmt_num(entry[f"{metric}_mean"], entry[f"{metric}_std"], digits=digits))
        rows.append(row)
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    difficulty_map = load_difficulty_map(tasks_dir)
    print(f"Loaded difficulty levels for {len(difficulty_map)} tasks from {tasks_dir}.")

    by_model_app, eff_by_model, eff_by_model_app, eff_by_model_diff = aggregate(
        triples, difficulty_map=difficulty_map
    )

    fsr_path  = args.out / "success_rate.csv"
    eff_path  = args.out / "efficiency.csv"
    diff_path = args.out / "efficiency_diff.csv"

    write_success_csv(fsr_path, by_model_app)
    write_efficiency_csv(eff_path, eff_by_model, by_model_app)
    write_efficiency_diff_csv(diff_path, eff_by_model, eff_by_model_diff, by_model_app)
    per_app_paths = write_efficiency_per_app_csvs(args.out, eff_by_model_app, by_model_app)

    written = "\n  ".join(str(p) for p in [fsr_path, eff_path, diff_path, *per_app_paths])
    print(f"\nWrote:\n  {written}")
    return 0
