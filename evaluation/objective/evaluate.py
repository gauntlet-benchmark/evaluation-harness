...
def run_evaluation_from_file(
    ...
    Args:
        tasks_path: Path to YAML tasks file.
    ...
    for i, tc in enumerate(tasks, 1):
        ...
        console.print(f"\n[bold cyan]Test {i}/{len(tasks)}: {test_id}[/bold cyan]")
...
""",
    ...
    parser.add_argument(
        "--tasks", required=True, type=str,
        help="Path to YAML tasks file",
    )
    parser.add_argument(
