...
def normalize(value: Any) -> str:
    """Lowercase and strip whitespace from a value's string form."""
...
def _strip_unit_suffix(text: str) -> str:
    ...
    '386 kts' -> '386', '2.605 V' -> '2.605', '-1.5 V' -> '-1.5',
    '58%' -> '58', '191.615 mV' -> '191.615', 'abc' -> 'abc' (unchanged),
    '16:35 UTC' -> '16:35 UTC' (unchanged — times are not numeric+unit).
    Handles both space-separated and directly-attached units; also handles
    negative values and the '%' symbol.
    """
    m = re.match(r"^(-?[\d,.]+)(?:\s*[A-Za-z%]+)?\s*$", text.strip())
    return m.group(1) if m else text


def compare_values(actual: Any, expected: Any, rel_tol: float = 0.0) -> bool:
    ...
    Scalars are normalized (lowercase, stripped) before comparison; trailing
    unit suffixes (e.g. 'V', 'Hz', '%') are stripped before numeric comparison.
    When *rel_tol* > 0, numeric values are accepted if their relative difference
    is within that tolerance (e.g. rel_tol=0.05 allows ±5%).

    Args:
        actual: Value from agent response.
        expected: Ground truth value.
        rel_tol: Maximum allowed relative error for numeric comparisons (0 = exact).
    ...
    if isinstance(expected, dict):
        if not isinstance(actual, dict):
            return False
        return all(compare_values(actual.get(k), v, rel_tol) for k, v in expected.items())
    ...
    if norm_exp and norm_act and norm_exp == norm_act:
        return True

    if rel_tol > 0 and norm_exp and norm_act:
        try:
            exp_f = float(norm_exp.replace(",", ""))
            act_f = float(norm_act.replace(",", ""))
            if exp_f == 0:
                return act_f == 0
            if abs(act_f - exp_f) / abs(exp_f) <= rel_tol:
                return True
        except ValueError:
            pass
