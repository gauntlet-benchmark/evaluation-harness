...
def load_tolerance_overrides(gt_dir: Path) -> dict[int, float]:
    """Load per-task tolerance overrides from tolerance_overrides.json."""
    overrides_path = gt_dir / "tolerance_overrides.json"
...
def evaluate_single(
    ...
    gt_dir: Path,
    tolerance: float,
    tolerance_overrides: dict[int, float] | None = None,
) -> EvalResult:
    ...
    if tolerance_overrides and task_number in tolerance_overrides:
        tolerance = tolerance_overrides[task_number]
...
def evaluate_all(
    ...
    """Evaluate all 3D editor tasks from a results directory.
    ...
    tolerance_overrides = load_tolerance_overrides(gt_path)
    ...
    for response in responses:
        ...
        result = evaluate_single(test_id, content, gt_path, tolerance, tolerance_overrides)
        total += 1
