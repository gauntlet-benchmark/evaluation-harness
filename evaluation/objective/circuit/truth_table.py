...
# Logic components that are recognised during gate-resolution phase.
# "g" (ground) is treated as a constant-0 driver.
LOGIC_COMPONENT_CODES = {"L", "M", "I", "g", "150", "151", "152", "153", "154", "155"}
# SPST/SPDT switches used as either logic inputs or output probes.
SWITCH_CODES = {"s", "S"}
    ...
    def evaluate_outputs(
        ...
        def eval_component(cid: str) -> bool:
            ...
            if code == "L":
                ...
                value = False  # ground = logic 0
            elif code == "I":
                value = not eval_net(required_net(cid, "in1"))
            elif code in BINARY_GATE_CODES:
                # Collect all input pins (in1, in2, ..., inN) sorted by index.
                input_nets = sorted(
                    (pin, net)
                    for pin, net in self.pin_to_net[cid].items()
                    if pin.startswith("in")
                )
                if not input_nets:
                    raise TruthTableError(f"gate {cid} has no input pins connected")
                vals = [eval_net(net) for _, net in input_nets]
                if code == "150":
                    value = all(vals)
                elif code == "151":
                    value = not all(vals)
...
def _build_logic_circuit(export_text: str) -> LogicCircuit:
    ...
    all_handled_codes = LOGIC_COMPONENT_CODES | SWITCH_CODES
    unsupported_codes = sorted({
        code for code in component_codes.values()
        if code not in all_handled_codes
    })
    ...
    def _raw_pins(cid: str) -> dict[str, str]:
        pins: dict[str, str] = {}
        for net_id in bipartite.neighbors(cid):
            for pin_name in bipartite[cid][net_id].get("pins", []):
                pins[pin_name] = net_id
        return pins

    # Phase 1: process non-switch components (gates, L, M, I, g).
    for cid, code in component_codes.items():
        if code in SWITCH_CODES:
            continue

        comp_pins = _raw_pins(cid)
        pin_to_net[cid] = comp_pins

        required_pins = REQUIRED_PINS.get(code, set())
        missing = sorted(required_pins - set(comp_pins))
        if missing:
            raise TruthTableError(
                f"component {cid} ({code}) missing pins: {', '.join(missing)}"
            )

        for pin_name, net_id in comp_pins.items():
            if pin_name in DRIVER_PINS.get(code, set()):
                net_drivers[net_id].append(cid)

    # Phase 2: classify switch (s/S) components as inputs or outputs.
    #
    # A switch is an OUTPUT if its left terminal (p1) sits in a net already
    # driven by a gate or constant (g). Otherwise it is treated as an INPUT
    # that drives its right terminal (p2) into the circuit.
    #
    # Sub-phase 2a: gate/constant-driven nets → OUTPUT immediately.
    # Sub-phase 2b: remaining → tentatively INPUT; register p2 as driver.
    # Sub-phase 2c: re-check tentative inputs whose p1 ended up in a
    #               switch-input-driven net (e.g. output indicator wired
    #               directly to an input switch) → reclassify as OUTPUT.

    switch_cids = [cid for cid, code in component_codes.items() if code in SWITCH_CODES]

    remaining: list[tuple[str, str | None, str | None]] = []
    for cid in switch_cids:
        raw = _raw_pins(cid)
        p1_net = raw.get("p1")
        p2_net = raw.get("p2")
        if p1_net and net_drivers.get(p1_net):
            component_codes[cid] = "M"
            pin_to_net[cid] = {"in1": p1_net}
        else:
            remaining.append((cid, p1_net, p2_net))

    for cid, p1_net, p2_net in remaining:
        component_codes[cid] = "L"
        if p2_net is not None:
            pin_to_net[cid] = {"out": p2_net}
            net_drivers[p2_net].append(cid)
        else:
            pin_to_net[cid] = {}

    for cid, p1_net, p2_net in remaining:
        if component_codes[cid] == "M":
            continue
        other_drivers = [d for d in net_drivers.get(p1_net or "", []) if d != cid]
        if other_drivers:
            if p2_net is not None and cid in net_drivers.get(p2_net, []):
                net_drivers[p2_net].remove(cid)
            component_codes[cid] = "M"
            pin_to_net[cid] = {"in1": p1_net}

    # Validate required pins for reclassified switches.
    for cid, code in component_codes.items():
        if code not in SWITCH_CODES:
            continue
        required_pins = REQUIRED_PINS.get(component_codes[cid], set())
        missing = sorted(required_pins - set(pin_to_net.get(cid, {})))
        if missing:
            raise TruthTableError(
