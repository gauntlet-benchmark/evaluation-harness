...
# o = scope probe, x = text label: decorations with no electrical behavior.
# Both would otherwise register as components and fail the truth-table
# simulation ("unsupported logic components") even when the circuit is correct.
IGNORED_CODES = {"o", "x"}
...
def _gate_input_offsets(n: int) -> List[int]:
    """Vertical (or horizontal) pin offsets for an n-input logic gate.

    CircuitJS places gate inputs symmetrically around the gate centre with a
    step of 16 units, matching the wiring grid:
      n=2 → [-16, +16]
      n=3 → [-16,   0, +16]
      n=4 → [-32, -16, +16, +32]
    """
...
class Component:
    ...
    y2: int
    raw: str
    num_inputs: int = 2  # relevant for gate codes; 2 is the standard default
    ...
    def pin_points(self) -> Dict[str, Point]:
        ...
        if self.code in TWO_INPUT_GATE_CODES:
            offsets = _gate_input_offsets(self.num_inputs)
            pins: Dict[str, Point] = {}
            if self._is_horizontal():
                for i, off in enumerate(offsets):
                    pins[f"in{i + 1}"] = (self.x1, self.y1 + off)
            else:
                for i, off in enumerate(offsets):
                    pins[f"in{i + 1}"] = (self.x1 + off, self.y1)
            pins["out"] = (self.x2, self.y2)
            return pins
...
_XML_RW_RE = re.compile(
    r'<rw\s+x="(?P<coords>[^"]+)"[^>]*>[^<]*</rw>', re.DOTALL
)
_XML_IN_ATTR_RE = re.compile(r'\bin="(\d+)"')
...
def _parse_xml_export(
    ...
    for match in _XML_RW_RE.finditer(xml_export):
        try:
            coords = list(map(int, match.group("coords").split()))
        except ValueError:
            continue
        if len(coords) >= 4:
            # Only connect declared endpoints — intermediate waypoints are bends, not junctions
            wires.append(((coords[0], coords[1]), (coords[2], coords[3])))
    ...
    for match in _XML_ELEMENT_RE.finditer(xml_export):
        ...
        raw = match.group(0)
        num_inputs = 2
        if code in TWO_INPUT_GATE_CODES:
            m = _XML_IN_ATTR_RE.search(raw)
            if m:
                num_inputs = max(2, int(m.group(1)))
        components.append(
            Component(
                cid=f"c{comp_idx}",
                code=code,
