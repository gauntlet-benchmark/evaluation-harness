...
def _shift_export_to_zero(export: dict) -> dict:
    """Return a copy of export with the timeline shifted so the first block starts at 0.

    Adjusts startTimeMs on every element, and startTimeMs/endTimeMs on every
    effectSubTrack item (including params.fadeStartMs / params.fadeEndMs).
    """
    import copy

    blocks = _flatten_timeline_blocks(export)
    if not blocks:
        return export
    offset = min(_to_float(b.raw.get("startTimeMs", 0)) for b in blocks)
    if offset == 0.0:
        return export

    export = copy.deepcopy(export)
    for track in export.get("timeline", []):
        for element in track.get("elements", []):
            if _is_number(element.get("startTimeMs")):
                element["startTimeMs"] = element["startTimeMs"] - offset
        for item in track.get("effectSubTrack", {}).get("items", []):
            for key in ("startTimeMs", "endTimeMs"):
                if _is_number(item.get(key)):
                    item[key] = item[key] - offset
            params = item.get("params")
            if isinstance(params, dict):
                for key in ("fadeStartMs", "fadeEndMs"):
                    if _is_number(params.get(key)):
...
def _validate_scenario_effects(
    ...
    errors: list[str] = []
    if scenario not in {7, 8, 9, 11, 14}:
        return errors
    ...
    if scenario == 11:
        ...
        if flower is None:
            errors.append("missing clip: Flower Video")
            return errors
        clip_end = _to_float(flower.raw.get("startTimeMs")) + _to_float(flower.raw.get("durationMs"))
        ...
        expected_fade_start = clip_end - 3000.0
        if abs(fade_end - clip_end) > epsilon_ms:
            errors.append("fade-out must end at clip end")
        if abs(fade_start - expected_fade_start) > epsilon_ms:
            errors.append(f"fade-out must start around clip_end-3000ms (expected ~{expected_fade_start:.0f}ms, got {fade_start:.0f}ms)")
        return errors
...
# Content compared on TEXT blocks, only for fields the ground truth sets.
# Without this a title with the wrong words or colors still matched on timing.
TEXT_CONTENT_FIELDS = ("text", "fontColor", "backgroundColor")


def _hex_to_rgb(value: Any) -> tuple[int, int, int] | None:
    """Parse #rgb, #rgba, #rrggbb or #rrggbbaa; alpha is dropped."""
    text = str(value or "").strip().lower()
    digits = text[1:]
    if not text.startswith("#") or not digits or not set(digits) <= set("0123456789abcdef"):
        return None
...
def _basic_color_name(rgb: tuple[int, int, int]) -> str:
    """Bucket a color into the plain names task prompts use ("black", "green", ...)."""
    hue, saturation, value = colorsys.rgb_to_hsv(*(c / 255 for c in rgb))
    if value < 0.2:
        return "black"
    if saturation < 0.2:
        return "white" if value > 0.8 else "gray"
    degrees = hue * 360
    for limit, name in ((15, "red"), (45, "orange"), (70, "yellow"), (170, "green"),
                        (200, "cyan"), (260, "blue"), (345, "purple")):
        if degrees < limit:
            return name
    return "red"


def _colors_match(gt_value: Any, pred_value: Any) -> bool:
    """Match colors by basic name, ignoring alpha, not by exact hex.

    Prompts name colors ("black background", "green") while ground truths store
    one concrete shade per name (black as #00000080, green as #00ff00), and
    correct agents pick other shades of the same name (#000000, #008800).
    """
    gt_rgb, pred_rgb = _hex_to_rgb(gt_value), _hex_to_rgb(pred_value)
    if gt_rgb is None or pred_rgb is None:
        return str(gt_value).strip().lower() == str(pred_value or "").strip().lower()
    return _basic_color_name(gt_rgb) == _basic_color_name(pred_rgb)


def _compare_text_fields(gt_block: FlatBlock, pred_block: FlatBlock) -> str | None:
    for field in TEXT_CONTENT_FIELDS:
        ...
        pred_value = pred_block.raw.get(field)
        if field == "text":
            same = str(gt_value).strip() == str(pred_value or "").strip()
        else:
            same = _colors_match(gt_value, pred_value)
        if not same:
            return f"field '{field}' mismatch (gt={gt_value!r}, pred={pred_value!r})"
    return None
...
def _candidate_mismatch_reason(
    ...
    if gt_media_type == "TEXT":
        text_error = _compare_text_fields(gt_block, pred_block)
        if text_error:
            return text_error
    return ""
...
def evaluate_binary(scenario: int, gt_data: dict, result_data: dict, epsilon_ms: float) -> EvalResult:
    ...
    pred_export = _shift_export_to_zero(pred_export)
...
def evaluate_all(
    ...
    """Evaluate all tasks from a run_agent response file against ground truths.
    ...
    for response_entry in agent_responses:
        ...
        # Ground truth is normally {"scenario": N, "endpoint_content": {...}},
        # but some suites store the bare endpoint payload {"operations": [...]}.
        if "endpoint_content" in gt_entry:
            gt_endpoint = gt_entry["endpoint_content"]
        elif "operations" in gt_entry:
            gt_endpoint = gt_entry
        else:
            gt_endpoint = {}
