...
from the flightradar tasks YAML file.  The agent's final answer is read
from the ``primary_output`` field in ``agent_outputs.json``.  Ground-truth
answers live in ``tasks/flightradar.yaml`` under each test case's ``gt``
field (JSON-encoded string under the ``answer`` key).

Usage:
    python -m evaluation.objective.eval_flightradar <results_dir> [<tasks_yaml>] [--verbose]

Examples:
    python -m evaluation.objective.eval_flightradar final_results/flightradar/gemini31pro
    python -m evaluation.objective.eval_flightradar final_results/flightradar/opus46 tasks/flightradar.yaml --verbose
"""
...
def evaluate_all(
    results_dir: str,
    tasks_yaml: str = "tasks/flightradar.yaml",
    verbose: bool = False,
) -> None:
    """Evaluate all Flightradar tasks from a results directory.

    Args:
        results_dir:      Path to the model's benchmark results directory
                          (e.g. final_results/flightradar/gemini31pro).
        tasks_yaml:  Path to the flightradar tasks YAML file.
    ...
    if not yaml_path.exists():
        print("ERROR: tasks YAML not found: %s" % yaml_path, file=sys.stderr)
        sys.exit(1)

    tasks = load_tasks(str(yaml_path))
    gt_by_id = {tc["id"]: tc for tc in tasks}
