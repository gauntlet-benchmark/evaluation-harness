...
def _score_json_test(
    ...
    # Unwrap redundant nesting, two patterns:
    #   {"key": "{\"key\": \"value\"}"} → {"key": "value"}   (dict-in-string)
    #   {"key": "\"value\""}           → {"key": "value"}   (string-in-string)
        ...
        if isinstance(val, str):
            try:
                ...
                if isinstance(inner, dict) and key in inner:
                    prediction[key] = inner[key]
                elif isinstance(inner, str):
                    prediction[key] = inner
    ...
    matched = sum(
        ...
        if compare_values(prediction.get(key), expected, rel_tol=0.05)
    )
...
def evaluate_circuit_schemes(
    ...
    for tc in tasks:
        ...
        # Tasks with no run in this directory are left out of the score entirely
        # rather than counted as failures.  The other four evaluators iterate the
        # discovered run directories, so scoring the full task list here would
        # silently deflate a model that simply has not been run on every task.
        if test_id not in run_data:
            no_run += 1
            continue

        run_entry = run_data[test_id]
    ...
    console.print(f"Tasks scored: {len(similarities)} (xml={xml_count}, json={json_count})")
    console.print(f"Skipped: {skipped} (unrecognized GT), {no_run} (no run in this directory)")
...
""",
    ...
    parser.add_argument(
        "--tasks",
        type=str,
        default="tasks/circuit.yaml",
        help="Path to the circuit YAML tasks file",
    )
    parser.add_argument(
        "--responses",
        required=True,
