...
class JudgeConfig:
    ...
    stage3_anthropic_api_key: str | None = None
...
def run_pipeline(
    ...
    _require_objective_results_for_runs(run_dirs, objective_evaluation_results, objective_evaluation_path)
...
def _require_objective_results_for_runs(
    run_dirs: list[Path],
    objective_evaluation_results: dict[str, int],
    objective_evaluation_path: Path,
) -> None:
    """Fail before any model call if a run has no objective score.

    Checked up front rather than per run so the pipeline exits without spending
    stage 2 / stage 3 API calls on a batch it cannot score consistently.
    """
    missing: list[str] = []
    for run_dir in run_dirs:
        task_id = detect_task_id(run_dir, read_json_if_exists(run_dir / "summary_info.json"))
        if not task_id:
            missing.append(f"{run_dir.name} (no task id)")
        elif task_id not in objective_evaluation_results:
            missing.append(f"{run_dir.name} ({task_id})")

    if missing:
        preview = "\n  ".join(missing[:10])
        more = f"\n  ... and {len(missing) - 10} more" if len(missing) > 10 else ""
        raise RuntimeError(
            f"{len(missing)} run(s) have no objective evaluation score in "
            f"{objective_evaluation_path}:\n  {preview}{more}\n"
            "Re-run the objective evaluator for this directory so stage 3 has its prior."
        )
...
def process_run(
    ...
    if testcase is not None and final_screenshot_path is not None:
        # Raised outside the except below on purpose: a missing prior is a setup
        # error to surface, not a per-run judgement failure to record and skip.
        if objective_evaluation_result is None:
            raise RuntimeError(
                f"No objective evaluation score for task {task_id or '<unknown>'} "
                f"(run {run_dir.name}); refusing to judge without the stage 3 prior."
            )
        try:
...
def judge_final_outcome(
    ...
    if _is_claude_model(config.stage3_model):
        api_key = config.stage3_anthropic_api_key or os.environ.get("ANTHROPIC_API_KEY")
        if not api_key:
            raise RuntimeError("ANTHROPIC_API_KEY is required for Claude stage3 models unless --dry-run is used.")
        payload = {
                    ...
                    "content": [
                        {"type": "text", "text": user_prompt},
                        {
                            ...
                            "source": {
                                ...
                                "data": base64.b64encode(final_screenshot_path.read_bytes()).decode("ascii"),
                            },
                        },
                    ],
                }
            ...
            "tools": [
                {
                    ...
                    "description": "Return the final outcome judgment.",
                    "input_schema": stage3_json_schema(),
                }
            ],
            "tool_choice": {"type": "tool", "name": "stage3_final_outcome_judgment"},
        }
        response_json = _post_anthropic_messages_api(
            payload=payload,
            api_key=api_key,
            timeout_seconds=config.timeout_seconds,
        )
        output_text = _extract_anthropic_tool_result(response_json)
    else:
        api_key = config.api_key or os.environ.get("OPENAI_API_KEY")
        if not api_key:
            raise RuntimeError("OPENAI_API_KEY is required unless --dry-run is used.")
        payload = {
            ...
            "reasoning": {"effort": config.stage3_reasoning_effort},
            "text": {
                "format": {
                    "type": "json_schema",
                    "name": "stage3_final_outcome_judgment",
...
def parse_simple_testcases_yaml(path: Path) -> list[dict[str, str]]:
    ...
    if yaml is not None:
        ...
        for raw in raw_items:
            if not isinstance(raw, dict):
                continue
            items.append({k: ("" if v is None else str(v)) for k, v in raw.items()})
        return items
    ...
    while index < len(lines):
        line = lines[index]
        stripped = line.strip()
        if not stripped or stripped.startswith("#") or stripped == "tasks:":
            index += 1
            continue
...
def load_objective_evaluation_results(
    ...
    """Load the objective scores that stage 3 uses as its prior.

    Raises if they cannot be found or are empty: stage 3's rubric leans on this
    value and refuses a score of 5 when it is 0, so running without it silently
    changes the scoring regime.
    """
    path = resolve_objective_evaluation_path(results_dir=results_dir, configured_path=configured_path)
    if path is None:
        raise RuntimeError(
            f"No objective_evaluation.json for {results_dir}. Run the objective evaluator "
            "first (python -m evaluation.objective.batch_evaluate <root>), or pass "
    ...
    if not parsed:
        raise RuntimeError(f"No usable task scores parsed from {path}.")
...
def resolve_objective_evaluation_path(results_dir: Path, configured_path: Path | None) -> Path | None:
    """Locate the objective scores for exactly this results directory.

    Only the explicitly configured path and the file inside the results
    directory are considered.  Earlier versions also probed the parent
    directory and the repo root, which could silently score one model's runs
    against another model's objective results.
    """
    candidate_paths: list[Path] = []
    if configured_path is not None:
        candidate_paths.append(configured_path)
    candidate_paths.append(results_dir / "objective_evaluation.json")
...
def parse_objective_evaluation_entry(raw_key: Any, raw_value: Any) -> tuple[str | None, int]:
    if isinstance(raw_key, str):
        # [a-z0-9]+ so app segments containing digits match too — `tc_3d_001` and
        # `tc_clone3d_004` were silently dropped by an [a-z]+ class, which left
        # every 3D run without an objective prior at stage 3.
        match = re.search(r"(tc_[a-z0-9]+_\d+)\s*:?\s*([01])?\s*$", raw_key.strip())
...
def post_responses_api(
    ...
    return _post_with_retry(endpoint=endpoint, data=data, headers=headers, timeout_seconds=timeout_seconds, provider="OpenAI")
    ...
    while True:
        ...
        try:
            ...
            if retryable and attempt < max_retries:
                ...
                LOGGER.warning("%s API %d on attempt %d/%d, sleeping %.1fs", provider, exc.code, attempt + 1, max_retries, delay)
                time.sleep(delay)
                attempt += 1
                continue
            raise RuntimeError(f"{provider} API request failed ({exc.code}): {body}") from exc
        except urllib.error.URLError as exc:
            if attempt < max_retries:
                delay = min(60.0, (2 ** attempt) + random.uniform(0, 1))
                LOGGER.warning("%s API URLError %s on attempt %d/%d, sleeping %.1fs", provider, exc.reason, attempt + 1, max_retries, delay)
                time.sleep(delay)
                attempt += 1
                continue
            raise
...
def _post_anthropic_messages_api(
    payload: dict[str, Any],
    api_key: str,
    timeout_seconds: int,
) -> dict[str, Any]:
...
def _extract_anthropic_tool_result(response_json: dict[str, Any]) -> str:
    for content_item in response_json.get("content", []):
        if content_item.get("type") == "tool_use":
            return json.dumps(content_item.get("input", {}))
    raise RuntimeError(f"Could not extract tool_use result from Anthropic response: {json.dumps(response_json)[:1000]}")
