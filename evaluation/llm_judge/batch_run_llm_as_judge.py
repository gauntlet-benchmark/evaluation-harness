...
# Canonical app keys — the directory names used under seed_runs/ and the stems
# of the task files in tasks/.
APP_DIRS = ("3d", "circuit", "flightradar", "graph", "video")
DEFAULT_SUMMARY_CSV_NAME = "llm_judge_batch_summary.csv"
DEFAULT_RUNS_CSV_NAME = "llm_judge_batch_runs.csv"
# A dry run writes placeholder judgments. Under the real output name those are
# indistinguishable from scored ones to every downstream reader, and writing
# them into a results directory destroys whatever judgments were already there.
# Prefixing the filename keeps `--dry-run` genuinely read-only for real data.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="APP=PATH",
        help=(
            "Task YAML for an app, repeatable. Required for every app present under "
            "input_dir, e.g. --task-yaml 3d=tasks/3d.yaml --task-yaml graph=tasks/graph.yaml. "
            "App keys are normalized, so frad/flightradar and video/vid/voidcut are equivalent."
        ),
    )
    parser.add_argument(
        "--skip-existing",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip the OpenAI API and emit placeholder judgments. Output is written to "
            f"'{DRY_RUN_PREFIX}<output-name>' and the CSVs get the same prefix, so a dry "
            "run can never overwrite real judgments."
        ),
    )
...
def main() -> int:
    ...
    if not discovered:
        parser.error(
            f"No <app>/<model> results directories found under {args.input_dir}. "
            f"Expected a tree like <root>/[run*/]{{{','.join(APP_DIRS)}}}/<model>/."
        )

    # A dry run must not be able to clobber real results, so it writes under a
    # distinct name unless the caller named the output file explicitly.
    prefix = DRY_RUN_PREFIX if args.dry_run else ""
    jobs = build_jobs(
        results_dirs=discovered,
        input_dir=args.input_dir,
        output_name=prefix + args.output_name,
        task_yaml_overrides=task_yaml_overrides,
        parser=parser,
    )
    summary_csv = args.summary_csv or (args.input_dir / (prefix + DEFAULT_SUMMARY_CSV_NAME))
    runs_csv = args.runs_csv or (args.input_dir / (prefix + DEFAULT_RUNS_CSV_NAME))
...
def discover_results_dirs(input_dir: Path) -> list[Path]:
    """Find every ``[run*/]<app>/<model>`` results directory under *input_dir*.

    Directories are identified structurally, by their parent being one of the
    benchmark apps, rather than by any filename convention.  Symlinks are
    skipped so aliases of the app/model tree cannot queue a directory twice.
    """
    if is_app_model_dir(input_dir):
        return [input_dir.absolute()]

    found: set[Path] = set()
    # Bounded globs so we never descend into the run directories themselves,
    # each of which holds hundreds of screenshots and step pickles.
    for pattern in ("*/*", "run*/*/*"):
        for path in input_dir.glob(pattern):
            if is_app_model_dir(path):
                found.add(path.absolute())
    return sorted(found)


def is_app_model_dir(path: Path) -> bool:
    """True for ``<app>/<model>`` where <app> is one of the benchmark apps."""
    return (
        path.is_dir()
        and not path.is_symlink()
        and normalize_app_key(path.parent.name) in APP_DIRS
    )


def identify_results_dir(results_dir: Path) -> tuple[str, str]:
    """Return ``(model, app)`` for a results directory."""
    return results_dir.name, results_dir.parent.name
...
def build_jobs(
    ...
    for index, results_dir in enumerate(results_dirs, start=1):
        ...
        if task_yaml is None:
            _, app = identify_results_dir(results_dir)
            parser.error(
                f"No task YAML given for app {normalize_app_key(app)!r} "
                f"(from {results_dir}). Pass --task-yaml {normalize_app_key(app)}=tasks/<app>.yaml."
            )
        jobs.append(
            BatchJob(
                index=index,
...
def resolve_task_yaml(results_dir: Path, task_yaml_overrides: dict[str, Path]) -> Path | None:
    _, app = identify_results_dir(results_dir)
    return task_yaml_overrides.get(normalize_app_key(app))


def normalize_app_key(app_name: str) -> str:
    """Map any spelling of an app onto its canonical name.

    The canonical names are the directory names under ``seed_runs/`` and the
    task-file stems in ``tasks/``, so a resolved key can be used for both
    without translation.  Aliases exist only to keep ``--task-yaml`` forgiving.
    """
    normalized = app_name.strip().lower().replace("-", "").replace("_", "")
    if normalized in {"circuit", "circuitdesigner"}:
        return "circuit"
    if normalized in {"frad", "flightradar", "flightradar24", "flightradarapp"}:
        return "flightradar"
    if normalized in {"video", "vid", "voidcut", "videoeditor"}:
        return "video"
    if normalized in {"3d", "clone3d", "3deditor", "3dmodeller"}:
        return "3d"
    if normalized in {"graph", "workflow", "workflowbuilder"}:
        return "graph"
    return normalized
        ...
        rows.append(
            {
                ...
                "error": str(summary_info.get("error", "")),
...
def extract_path_metadata(results_dir: Path, input_dir: Path) -> tuple[str, str, str, str]:
    ...
    model, app = identify_results_dir(results_dir)
