"""Batch launcher for run_gemini.py config files."""
...
DEFAULT_ENTRYPOINT = Path(__file__).resolve().parent / "run_gemini.py"
...
IGNORED_FOR_GEMINI = {"reasoning", "reasoning_effort", "thinking_budget", "provider", "use_screenshot", "seed"}
...
def _coerce_experiment(raw: dict, base_dir: Path, model_name: str, index: int) -> ExperimentSpec:
    ...
    values: dict[str, object] = {
        ...
        "continue_run": raw.get("continue_run", False),
    ...
    values["continue_run"] = _ensure_bool(values["continue_run"], "continue_run")
...
def load_batch_config(config_path: Path) -> BatchConfig:
    ...
    for model_name, model_block in models.items():
        ...
        model_skip_existing_results = None
...
def build_gemini_command(spec: ExperimentSpec, entrypoint: Path, inline_js_path: str | None = None) -> list[str]:
    ...
    elif spec.js_snippet:
        command.extend(["--js-snippet-file", inline_js_path or "__INLINE_JS_SNIPPET_FILE__"])
...
class BatchRunner:
    ...
    def launch_experiment(self, spec: ExperimentSpec) -> None:
        ...
        inline_js_path = None
        if spec.js_snippet and not spec.post_run_js_snippet_path:
            with tempfile.NamedTemporaryFile(
                mode="w",
                suffix=".js",
                prefix="gemini_batch_inline_js_",
                delete=False,
                encoding="utf-8",
            ) as handle:
                handle.write(spec.js_snippet)
                inline_js_path = handle.name
            temp_files.append(Path(inline_js_path))

        command = build_gemini_command(spec, self.config.entrypoint, inline_js_path=inline_js_path)
    ...
    def poll_finished(self) -> list[FinishedExperiment]:
        ...
        for name, running in list(self.running.items()):
            ...
            self._cleanup_temp_files(running.temp_files)
...
def run_from_config(
    *,
    ...
    del use_screenshot
    batch_config = load_batch_config(Path(config).expanduser().resolve())

    if max_parallel is not None:
        if max_parallel < 0:
            raise ValueError("--max_parallel must be 0 or greater.")
        batch_config.max_parallel = max_parallel or len(batch_config.experiments)

    if fail_fast:
        batch_config.fail_fast = True

    if continue_run:
        for spec in batch_config.experiments:
            spec.continue_run = True

    enabled_experiments = [spec for spec in batch_config.experiments if spec.enabled]
    ...
    for spec in enabled_experiments:
        if should_skip_existing_results(
            spec,
            batch_config,
            force_skip_existing_results=skip_existing_results,
        ) and experiment_has_existing_results(spec):
    ...
    if dry_run:
        ...
        for spec in enabled_experiments:
            print(f"\n[{spec.name}]")
            print(shlex.join(build_gemini_command(spec, batch_config.entrypoint)))
        return 0

    runner = BatchRunner(batch_config, force_skip_existing_results=skip_existing_results)
...
def run_batch(config_path: str, max_parallel: int | None = None) -> int:
    """Backward-compatible alias for existing callers."""
    return run_from_config(config=config_path, max_parallel=max_parallel)
