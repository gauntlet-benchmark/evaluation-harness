...
def _coerce_experiment(raw: dict, base_dir: Path, model_name: str, index: int) -> ExperimentSpec:
    ...
    if values["thinking_type"] is not None:
        t = _ensure_string(values["thinking_type"], "thinking_type")
        valid_types = {"enabled", "adaptive", "disabled"}
        if t not in valid_types:
            raise ValueError(f"'thinking_type' must be one of {sorted(valid_types)}.")
        values["thinking_type"] = t
...
def build_main_command(spec: ExperimentSpec, entrypoint: Path) -> list[str]:
    ...
    if spec.thinking_type is not None:
        command.extend(["--thinking-type", spec.thinking_type])
    if spec.provider:
