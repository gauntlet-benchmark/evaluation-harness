...
# Import under the installed package name, not `src.agisdk`. Everything inside
# the package imports itself as `agisdk`, so reaching it via `src.` would load a
# second copy and re-register every gym task.
...
def create_task_artifacts_table(record: dict) -> Table:
    ...
    table.add_row("Artifacts", f"[magenta]{', '.join(record['artifacts']) or '—'}[/magenta]")
...
def filter_tasks_by_id(tasks: list[dict], task_id: str) -> list[dict]:
    """Select tasks by id. Accepts a comma-separated list of ids.

    A list is what lets a resume re-run an arbitrary set of tasks: --task-range
    can only express a contiguous slice, and after a partial failure the gaps
    rarely are. Selected tasks keep their order in the task file, so a resumed
    run visits them in the same order the original did.
    """
    wanted = set()
    for part in task_id.split(","):
        ...
        if not part:
            continue
        wanted.add(part.split(".", 1)[1] if part.startswith(f"{TASK_SCOPE_NAME}.") else part)
...
ARTIFACT_FILES = ("summary_info.json", "experiment.log", "agent_outputs.json", "agent_output.txt")


def build_artifact_record(task_name: str, result: dict, iteration: int) -> dict:
    """Index one task run for the batch manifest.

    This is a pointer record, not a copy of the run: the page captures, agent
    responses and per-step files all live in the run directory itself, so the
    manifest records only where they are and how many there are.
    """
    ...
    if exp_dir and exp_dir.exists():
        step_state_count = sum(1 for _ in exp_dir.glob("step_*.pkl.gz"))
        screenshot_count = sum(1 for _ in exp_dir.glob("screenshot_step_*.png"))
        artifacts = [name for name in ARTIFACT_FILES if (exp_dir / name).exists()]
    ...
    return {
        ...
        "artifacts": artifacts,
        "step_state_count": step_state_count,
...
def main():
    ...
    parser.add_argument("--thinking-type", type=str, default=None, choices=["enabled", "adaptive", "disabled"], help="Override thinking type (enabled/adaptive/disabled). Takes precedence over --thinking-budget when set.")
    ...
    with Progress(
        ...
        for iteration in range(args.iterations):
            ...
            try:
                iteration_results = harness.run(tasks=task_names)

                for task_name, result in iteration_results.items():
                    all_iteration_results[task_name].append(result)
                    artifact_record = build_artifact_record(task_name, result, iteration)
                    ...
                    progress.console.print(
                        f"  [{status_color}][{status}][/] {task_name} -> {artifact_record['exp_dir']}",
                        highlight=False,
                    )
